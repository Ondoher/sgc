Package('SgcModule.Services', {
	Module : new Class({
		...
		initialize : function()
		{
			this.serviceName = 'sgc:module';
			this.importServices = 'share,modules,ui'.split(',');

			SYMPHONY.services.make(this.serviceName, this, this.implements, true);

			SAPPHIRE.application.listen('start', this.onStart.bind(this));
			SGC_MODULE.events.listen('start', this.onReady.bind(this));
			SGC_MODULE.events.listen('started', this.onStarted.bind(this));
		...
		{
			var bootstrap = SYMPHONY.services.subscribe('sgc-module:bootstrap');
		...
		onStarted : function()
		{
			SGC_MODULE.appId = this.getAppId(SGC_MODULE.pod);
		},
		...
		sharePresentationML : function(gameNbr, time)
		{
			...
			var results = {gameNbr: gameNbr, time: fullTime};

			var presentationML =`
					<entity>
						<table><tr>
							<td style="width: 200px"><img src="${thumbnail}" /></td>
							<td>
								<h1>${title}</h1>
								${blurb}
							</td>
